...
- **リバースプロキシ（Nginx）における画像静的アセットおよび LiteLLM UI ルーティングの追加**
  - 生成画像表示用の `/static/` パスを `backend` コンテナへルーティングし、画面上での画像読み込み失敗を解決。
  - LiteLLM Admin UI およびログインリダイレクト用パス（`/litellm/`, `/ui/`, `/litellm-asset-prefix/`）を Nginx に透過追加。

### 🛠️ 開発・運用向けツール・診断機能の強化

- **自動画像生成ボトルネック・コンポーネント疎通診断ツール (`scripts/diagnose_image_gen.py`) の新規追加**
  - 環境変数、バックエンドのインポート/呼び出しロジック、コンポーネント(SD API、local-sd-api、LiteLLM、SeaweedFS等)の疎通、GPU/VRAM、および Nginx プロキシタイムアウト設定を包括的に一括診断し、ボトルネックを特定するスクリプトを導入。
...
- **認証切れ/セッション破損時の壊れた JWT 自動クリア (PR #21)**
  - API から 401 Unauthorized を受信した際、または認証エラーページへ遷移した際に、壊れたローカルストレージの JWT トークン (`clearToken`) を消去し、ログイン画面へ誘導するガードを追加。
- **依存関係セキュリティ脆弱性の解消 (PR #19)**
  - 依存ライブラリ `pypdf` を `6.14.2` に更新し、既知の CVE 脆弱性 (CVE-2026-59938, CVE-2026-59937, CVE-2026-59935, CVE-2026-59936) を完全解消。

### 🎨 UI / UX 改善 (Upstream v0.5.0)

- **アカウントメニュー・モバイルメニューでのユーザー表示名描画 (PR #21)**
  - ログイン中の JWT ペイロードから `username` / `email` などの表示名を動的に読み込み、デスクトップ用アカウントメニューおよびモバイル用メニューに表示する機能を追加。

### ⚡ データベースおよびクエリのパフォーマンス高速化 (PR #20)

- **RAGノード一括取得のバッチ最適化 (`get_nodes_with_text`)**
  - ノードごとに個別に DB 接続していた処理を、単一接続内で `IN` 句およびページ範囲クエリを用いて一括取得するように最適化。500件単位の自動チャンク分割に対応し **約3.5倍高速化**。
- **RAGツリー探索およびチーム認可チェックの高速化**
  - Tree RAG 探索用に `(doc_id, parent_id)` 複合インデックス (`idx_tree_nodes_parent`) を追加（**約9.6倍高速化**）。
  - `team_users` および `exapps` テーブルにインデックスを追加し認可判定処理を最適化（**約1.4倍高速化**）。
