...
### SAML 認証フロー強化・リバースプロキシ連携および UI 表示名の対応

- **SAML リクエスト動的パース・例外ハンドリングの強化 (PR #18, #21)**
  - Nginx などのリバースプロキシ配下での ACS/SLS 転送に対応するため、`X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Forwarded-Port` ヘッダからリクエスト情報を正しくパースするよう改善。
  - Keycloak 等からの重複 Attribute 名を許可する `allowRepeatAttributeName: True` を SAML 設定に追加。
  - SAML 応答の検証失敗・例外発生時に、IdP メタデータキャッシュ (`reset_settings_cache()`) をクリアし、安全にログインエラー画面 (`/auth-error`) へリダイレクト。
- **フロントエンド認証切れ/セッション破損時の自動リカバリ**
  - 認証エラー到達時や 401 Unauthorized レスポンス受信時に、破損したローカルストレージの JWT トークン (`clearToken`) を自動破棄し、ログイン画面へ誘導するようガード処理を追加。
- **アカウントメニュー・モバイルメニューでのユーザー表示名レンダリング**
  - ログイン中の JWT ペイロードから `username` / `email` などの表示名を取り出し、ヘッダーのアカウントメニューおよびモバイルメニューに動的に描画する機能を追加。
- **依存関係セキュリティ脆弱性の解消**
  - 依存ライブラリ `pypdf` を `6.14.2` に更新し、既知の CVE 脆弱性 (CVE-2026-59938, CVE-2026-59937, CVE-2026-59935, CVE-2026-59936) を完全に解消。
