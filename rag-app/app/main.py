...
def _chunk_id(scope: str, source: str, text: str) -> str:
    # 同一(スコープ+出典+本文)のチャンクは同じ ID になり、upsert で上書き＝重複排除
...
async def ingest_documents(docs: list[dict[str, Any]], scope: str) -> int:
    """docs を埋め込んで、指定スコープ(teamId 等)に紐付けて Qdrant に登録。
...
def _is_admin(x_user_groups: str | None) -> bool:
    ...
    return "SystemAdminGroup" in groups
...
async def invoke(
    ...
    # ---- 管理操作: 出典一覧 / 出典削除 / 全消去（すべてスコープ内に限定） ----
    if action == "list_sources":
        srcs = await vectorstore.list_sources(scope)
        if not srcs:
            return {"outputs": "このチームの知識ベースは空です。"}
        lines = "\n".join(f"- {s['source']}（{s['chunks']} チャンク）" for s in srcs)
        return {"outputs": f"## 登録済みの出典（このチーム）\n\n{lines}"}
    ...
    if action == "delete_source":
        ...
        return {"outputs": f"出典「{source}」をこのチームの知識ベースから削除しました。"}
    ...
    try:
        ...
        if uploaded:
            await ingest_documents(uploaded, scope)
        qvec = await embeddings.embed(question, is_query=True)
        hits = await vectorstore.search(qvec, top_k, scope)
