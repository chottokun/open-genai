...
## 🛠️ Python 開発環境・依存関係の管理方針

1. **`uv` の優先利用**
   * Pythonの仮想環境（venv）の作成や依存関係（requirements.txtなど）のインストール、脆弱性監査（pip-audit等）の実行には、可能な限り `uv`（`uv venv`, `uv pip install`）を優先して使用すること。
   * これにより、Debian/Ubuntuなどのシステム環境（特に `python3-venv` が標準で入っていない環境など）における `ensurepip` の欠如による仮想環境作成エラー等を回避し、高速かつ安定したテスト・ビルド実行を可能にする。
   * 自動セットアップスクリプト等では、システム上に `uv` コマンドが存在するかどうかをチェックし、存在する場合には `uv` を、存在しない場合には `python3 -m venv` にフォールバックするハイブリッドなロジックを実装すること。
