...
* **画像生成（Stable Diffusion）の完全コンテナ化と段階的アップグレード (As-Is / To-Be)**:
  ...
  * **[設計指針: スモールスタートと段階的スケール]**:
    * **第1段階 (検証・軽量化フェーズ)**: CPU環境や最小構成でのインフラ稼働のため、デフォルトでは超軽量・高速な蒸留モデル **`latent-consistency/lcm-sd15`** (約2GB, 4ステップ推論) を指定。これにより、コンテナ起動速度とメモリ消費を極限まで抑えながら、チャット疎通や配管検証を高速に行うことができます。
