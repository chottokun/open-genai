...
* **画像生成（Stable Diffusion）の完全コンテナ化と段階的アップグレード (As-Is / To-Be)**:
  ...
  * **[多様な画像モデルへの動的対応]**:
    * `local-sd-api` は内部で万能な **`DiffusionPipeline`** (Diffusers) をローダーとして採用しています。これにより、コンテナ側のソースコードを一切改修することなく、環境変数 **`IMAGE_MODEL_NAME`** を書き換えるだけで、SD1.5系列、SDXL系列、および最新のFLUX系列などの多種多様なモデルを動的にダウンロード・起動ロードすることが可能です。
  * **[設計指針: スモールスタートと段階的スケール]**:
    * **第1段階 (検証・軽量化フェーズ)**: CPU環境や最小構成でのインフラ稼働のため、デフォルトでは認証不要で一般公開されている超軽量・高速な蒸留モデル **`SimianLuo/LCM_Dreamshaper_v7`** (約2GB, 4ステップ推論) を指定。これにより、コンテナ起動速度とメモリ消費を極限まで抑えながら、チャット疎通や配管検証を高速に行うことができます。
    * **第2段階 (本格実用化フェーズ)**: GPUを割り当てた環境にて、 **`SDXL-Lightning`** (4-Step/8-Step, 約5GB) に切り替えることで、低リソースながら実用的な高品質画像の高速生成を可能にします。
    * **第3段階 (プロダクションフェーズ)**: 最先端の画像生成モデル **`FLUX.1 [schnell]`** などを導入し、文字の描画や手の形状破綻がない商用クオリティの生成へと、バックエンドのソース修正不要（`.env` の `IMAGE_MODEL_NAME` の変更のみ）でシームレスに差し替え・スケールアップを行います。
  * **[LiteLLM Proxy のハブ（軸）化による抽象化]**:
    * 接続先を `open-genai-litellm` に集約しておくことで、裏側のモデルを `openai/sdxl-lightning` から `openai/flux-schnell` に切り替える際も、 `litellm` の `config.yaml` のルーティング定義を1箇所変更するだけで、フロントエンドやバックエンドの再ビルドを伴わずに安全にモデルリプレイスが完了します。
