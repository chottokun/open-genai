...
### 🎨 1.2 画像生成 (Stable Diffusion / 外部モデル) 系統

#### ① `backend` 内蔵画像生成 API (ルーティング層)
SPAフロントエンドから直接呼び出される、画像生成制御用の内蔵APIです。
このエンドポイントは、指定された `IMAGE_PROVIDER` の設定に応じて、LiteLLM（外部API）または `local-sd-api` コンテナ、もしくはホスト側の Stable Diffusion へリクエストをルーティングします。
...
```json
{
  "params": {
    "textPrompt": [
      {"text": "A beautiful white cat", "weight": 1},
...
##### レスポンス (text/plain - `200 OK`)
生成された最初の画像の Base64 エンコード文字列が直接返却されます。
```text
iVBORw0KGgoAAA...
```
...
## 🎯 3. AIハブ (LiteLLM) に寄せられる機能と寄せられない機能の境界定義

AI システム全体の保守性と安全性を高く保つため、「LiteLLM で一元管理（抽象化）する機能」と「個別の AI アプリ (exApps) やバックエンド側で実装すべき機能」の境界を以下のように明確に区別して設計します。

### 📥 3.1 LiteLLM に寄せられる（一元抽象化できる）機能
LiteLLM は **「AI モデル API へのアクセスを統一するスマートなプロキシゲートウェイ」** として機能します。

1. **モデル推論 API の統一**：
   * LLM チャット（OpenAI, Gemini, Ollama 等）、音声認識（Whisper 等）、画像生成（Imagen 4, Stable Diffusion 等）、テキスト埋め込み（ruri-v3-30m 等）などの異種プロバイダの API 規格を OpenAI 形式へ一本化。
2. **ルーティング ＆ 自動フォールバック（ハイブリッド運用）**：
   * クラウド API のレートリミット超過時や、API キー未設定時（ローカル検証等）におけるローカル推推エンジンへの自動フェイルオーバー・フォールバック。
3. **トラフィック・コスト制御**：
   * リトライ回数（`num_retries`）やリクエストタイムアウト、送信パラメータの自動クレンジング（`drop_params`）の集中管理。
   * トークン消費量や利用モデルなどの監査ログ（物理アクセスログ）の一元集約。

### 📤 3.2 LiteLLM に寄せられない（個別 AI アプリ / バックエンド側で制御すべき）機能
LiteLLM はステート（状態）や複雑な業務ワークフローを持たないため、以下のビジネスロジックやデータ操作は個別の AI アプリ層（exApps）でプログラムとして制御します。

1. **データの前処理・保存管理ワークフロー (RAG ロジック等)**：
   * ファイルアップロード、テキストのパース抽出、セマンティックなチャンク分割（分割サイズやオーバーラップ文字数算出）、ベクトルDB（Qdrant）やストレージ（SeaweedFS）への保存・削除トランザクション管理。
2. **認可とアクセススコープの論理隔離**：
   * バックエンド（`backend`）と連動した「どのユーザー / どのチームのデータスコープにアクセスできるか」の HMAC 署名検証および認可制御（`intauth`）。
3. **ビジネス要件に基づくコンテンツの加工**：
   * 送信前に個人情報（PII）の特定とマスク・置換（`[MASK]`）を行うなどの「組織固有の高度なセキュリティフィルタリングロジック」。
   * 生成されたバイナリデータ（画像等）のファイル永続化と署名付き URL 配信のためのアセット管理。
