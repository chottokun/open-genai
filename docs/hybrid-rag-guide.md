...
```mermaid
graph TD
    User([ユーザー]) -->|HTTP/HTTPS :80/443| Proxy[open-genai-proxy Nginx]
    Proxy -->|/| Web[genai-web Vite:5173]
    Proxy -->|/api/| Backend[backend FastAPI:8000]
    Proxy -->|/kc/| Keycloak[keycloak :8080]
    
    subgraph RAG アプリ構成
        Backend -->|RAGリクエスト| RAG[rag-app :8001]
        RAG -->|Embeddingリクエスト| LiteLLM[LiteLLM Proxy :4000]
...
## 💻 外部ホスト/ローカルドメインからの接続設定（リバースプロキシ構成）
...
> [!NOTE]
> 設定変更後、`docker compose up -d --force-recreate` を実行し、ブラウザ側でスーパーリロード（`Cmd+Shift+R` / `Ctrl+F5`）を行ってください。フロントエンド、バックエンド、および Keycloak への認証ACSルーティングが自動的にプロキシ配下のURLに解決されます。
...
### 3. ホスト名や外部公開ドメイン（`PUBLIC_URL`等）を変更した場合
アクセス元のURLや SAML 認証（Keycloak）の公開ホスト名を変更した場合は、プロキシ・認証・フロント・バックエンドの一連のコンテナを再起動します。

```bash
# 影響するコンテナを一斉に再起動して新しい環境変数を反映
docker compose up -d --force-recreate proxy web backend keycloak
```

---

## 🛡️ 品質検証とセキュリティテスト（TDD / CI-CD 前提スキャン）

開発環境（`dev` ブランチ）では、CI/CD でのテスト落ちや本番環境での脆弱性流出を防ぐため、徹底したテストとスキャンによる保護が行われています。

### 1. 自動テストの実行方法 (`pytest`)

TDD (テスト駆動開発) で構築されたセキュリティ防御テストおよび RAG 順序保証テストを実行するには、ホストの仮想環境にて以下を実行します。

```bash
# ① バックエンド（ワンタイム署名・パストラバーサル防御）のテスト実行
PYTHONPATH=.:backend .venv/bin/pytest backend/tests/

# ② RAGアプリ（非同期バッチ処理・並行順序保証）のテスト実行
PYTHONPATH=.:rag-app .venv/bin/pytest rag-app/tests/
```

### 2. 静的セキュリティ解析の実行方法 (`bandit`)

ソースコードに対するセキュリティ問題の混入を防ぐため、 `bandit` スキャンを定期的に実施します。

```bash
# 全 Python サービスのコードを一括静的解析
.venv/bin/bandit -r backend/app/ rag-app/app/ sd-app/app/ whisper-app/app/ audit-app/app/ modelpolicy-app/app/ ngword-app/app/ prompt-app/app/ usermgmt-app/app/
```
> [!NOTE]
> 例外スキップ処理やダミーの変数マッピング等の意図的な実装に対しては、コード内に `# nosec` を追加して安全にサプレスしています。

### 3. 依存関係の脆弱性スキャンの実行方法 (`pip-audit`)

新しく追加された python ライブラリや requirements に含まれるパッケージの脆弱性を検証します。

```bash
# 仮想環境内のパッケージ脆弱性をフルスキャン
.venv/bin/pip-audit
```
> [!IMPORTANT]
> `dev` ブランチでは、 `fastapi`, `starlette`, `PyJWT`, `pypdf`, `python-multipart`, `requests` の既知の脆弱性（計58件）を最新の修正バージョンに引き上げることで、**脆弱性・検出数ゼロ** の極めて安全な状態を維持しています。
