# 画像生成 (Stable Diffusion / LiteLLM) 構成・導入ガイド

本ドキュメントでは、OpenGENAI における画像生成（Stable Diffusion / 外部クラウドモデル）機能のアーキテクチャ、LiteLLM をハブとする自動フォールバック構成、ローカルモデルと外部クラウドモデルの切り替え、およびセキュリティ設定について解説します。

> 💡 **詳細アーキテクチャ・提言書のご案内**:
> LiteLLM 統合型画像生成のさらなる保守性向上、標準パラメータ仕様、および推論基盤の完全分離（デカップリング）に関する包括的な調査報告と中長期的な設計指針については、[LiteLLM統合型画像生成アーキテクチャおよび推論基盤分離に関する調査報告書](litellm-image-gen-architecture.md) をご参照ください。
...
OpenGENAI ではこれらをオープンかつローカル完結な仕組みに置き換え、さらに開発環境や実行リソースの有無に柔軟に対応するため、バックエンドへの画像生成ロジックの統合と、軽量な推論コンテナの組み合わせで提供しています。

1. **`backend` (画像生成処理の統合とルーティング)**:
   * バックエンド（`backend/app/image_gen.py`）にて直接 `IMAGE_PROVIDER` の設定（`local` / `litellm` / `local_api`）を判別し、外部APIやローカルコンテナを呼び分けるように一本化されています。
2. **`local-sd-api` (推論エンジン層 - 任意起動)**:
...
flowchart TD
    ...
    Backend -->|3. 中継 /v1/images/generations（imagen-4）| LiteLLM[open-genai-litellm :4000]
    
    %% LiteLLM内での判定分岐
    LiteLLM -->|4a. GEMINI_API_KEYがある場合| CloudAPI["Google Imagen 4 (高クオリティ生成)"]
    LiteLLM -->|4b. GEMINI_API_KEYがない場合（エラー発生）| Fallback["自動フォールバック (imagen-4 -> sd-local)"]
    
    Fallback -->|5. ローカル中継 /v1/images/generations| LocalSD[local-sd-apiコンテナ :8004]
    LocalSD -->|6. ローカル推論実行| SDModel[SimianLuo/LCM_Dreamshaper_v7]
    SDModel -->|7. Base64画像データを返却| LocalSD
    LocalSD -->|8. 結果返却| LiteLLM
    LiteLLM -->|9. 最終応答を返却| Backend
    Backend -->|10. 画面描画| Browser
```
...
* **ソース修正の不要**: フロントエンドやバックエンド of ソースコードは、常に共通の `imagen-4` モデルを呼び出すだけで良く、インフラ設定によってシームレスにスケールします。
...
| **高品質ローカル** | `ByteDance/SDXL-Lightning` (8-step) | 約 5 GB / ローカルGPU推奨 | 実用的な高解像度画像を高速生成可能。 |
| **プロダクション** | `black-forest-labs/FLUX.1-schnell` | 約 10 GB以上 / 高性能VRAM必須 | 手の形状や文字描画の破綻がない商用レベルのローカル画像生成。 |
...
* **FLUX.1 系 (例: `FLUX.1-schnell` など)**: 内部で最新の `FluxPipeline` を選択してロードする構造へ拡張されます。
...
### ② 画像生成機能の疎通・結合テスト
```bash
# クッキーや認証（セッション情報）を模して、直接 backend の画像生成APIを呼び出すテスト
# フロントエンドからのリクエストと同様の形式でテスト可能
curl -i -X POST \
  -H "Content-Type: application/json" \
  -d '{"params": {"textPrompt": [{"text": "A beautiful white cat", "weight": 1}], "width": 512, "height": 512}}' \
  http://localhost/api/image/generate
```
